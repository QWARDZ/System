    ...
    def setup_routes(self):
            ...
            if 'username' in session and session['role'] == 'admin':
                with self.mysql.connection.cursor() as cursor:
                    ...
                    cursor.execute("""
                        ...
                        FROM student_info
                        WHERE student_info.user_id = 0  -- Select students with no accounts yet (user_id = 0)
                    """)
        ...
        def insert_user(self, user_data):
            with self.mysql.connection.cursor() as cursor:
                ...
                cursor.execute("UPDATE student_info SET user_id = %s WHERE stud_id = %s", (user_id, user_data['student_id']))
                self.mysql.connection.commit()
